...
public class Chat extends Activity implements OnKeyListener{
	...
	Drawable image;
    Session s;
	String sessionid, myprofileid,profileid, url,data, email, password,message_data;
	String img = "http://www.quipmate.com/upload_pic/2isk92li26mi0b7m02e4kuvq421353130910719754526674026619136382609593295_t.jpg",actionby,msg;
	HttpResponse r;
	HttpPost post;
			...
			public void run(){
				try {
					...
					if(status == 200){
						...
						String myprofileid_data = new JSONObject(data).getString("myprofileid");
						...
						String photo_data = new JSONObject(data).getString("pimage");
					...
					System.out.println("Unable to reach Quipmate. Check Internet Connection !");
					e.printStackTrace();
			...
			public void run() {
				...
				while(k>=0){
					...
					try {
						...
						String actionby = new JSONObject(message.get(k).toString()).getString("actionby");						
						String msg = new JSONObject(message.get(k).toString()).getString("message");	
	        	...
	        	if(typing == true && v.getId()== R.id.etChatBox)
	        	{
	   			 ...
	   			 post = new HttpPost(url);
	   			    ...
	   			    try {
	   				    ...
	   				    nameValuePairs.add(new BasicNameValuePair("name", name.get(myprofileid).toString()));		    
			...
			System.out.println(msg);
			 ...
			 post = new HttpPost(url);
			    ...
			    try {
				    ...
				    nameValuePairs.add(new BasicNameValuePair("name", name.get(myprofileid).toString()));
				    nameValuePairs.add(new BasicNameValuePair("photo", pimage.get(myprofileid).toString()));			    
					post.setEntity(new UrlEncodedFormEntity(nameValuePairs));
				} catch (UnsupportedEncodingException e1) {
					e1.printStackTrace();
				} catch (JSONException e) {
